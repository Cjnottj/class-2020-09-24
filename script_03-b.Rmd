...
```{r}
joinedins <- sep_old %>%
  inner_join(sep_new, by = c("id", "title", "name", "department")) %>%
  #we joined on everything except u grad inorder to 
  mutate(diff = u_grad.y - u_grad.x) %>%
  arrange(desc(diff)) %>%
  slice(1:5)
...
x <-left_join(sep_old, sep_new,
              ...
              suffix = c("_old", "_new")) %>%
  mutate(change = u_grade_row - u_grad_old)
...
```{r}
ten_movers <- joinedins%>%
  full_join(joinedds %>% arrange(desc(u_grad_diff)),
            by = c("id", "title", "name", "department", "u_grad.x", "u_grad.y", "u_grad_diff"))

ten_movers %>%
  ggplot(mapping = aes(x = name, y = u_grad_diff)) +
  geom_col()
```
